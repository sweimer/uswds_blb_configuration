    ...
    attach: function (context) {
      ...
      $('.uswds_breakpoint_cols', context).each(function () {
        ...
        once('custom-control-custom-radio-bario',$(this).find('.form-item, .custom-control, .custom-radio'),context).forEach(function (elem,i) {
          ...
          var col_classes = 'uswds_breakpoint_col';
          ...
          colsLabel.wrapInner('<div class="uswds_tooltip uswds_tooltip-lg"></div>');
          ...
          $.each(colsConfig, function(index, value) {
            ...
            .appendTo(colsLabel)
            .on('click', function () {
              $(this).parents('.uswds_breakpoint_cols').find('.uswds_breakpoint_cols').removeClass('bp-selected');
              $(this).parents('.uswds_breakpoint_cols').find('input').prop("checked", false);
              $(this).parents('label').parent().find('input').prop("checked", true);
              $(this).parents('label').find('.uswds_breakpoint_col').addClass('bp-selected');
            });
